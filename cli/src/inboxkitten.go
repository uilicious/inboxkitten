...
func main() {
    ...
    if listCommand.Parsed() {
        ...
        var urlWithParams = api + "/mail/list?recipient=" + url.QueryEscape(email)
    ...
    if getCommand.Parsed() {
        ...
        // Validate and sanitize the region and key
        validRegion := validateParam(region)
        validKey := validateParam(key)
        if !validRegion || !validKey {
            fmt.Fprintf(os.Stderr, "Invalid region or key format\n")
            os.Exit(1)
        }

        var urlWithParams = api + "/mail/getKey?mailKey=" + url.QueryEscape(region+"-"+key)
        var body = doGetRequest(urlWithParams)
        fmt.Println(jsonPrettifier(string(body)))
    }
}
...
func validateParam(param string) bool {
    // Use a regular expression to validate the parameter format
    paramRegex := regexp.MustCompile(`^[a-zA-Z0-9._-]+$`)
    return paramRegex.MatchString(param)
}
